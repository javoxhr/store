    ...
    <div class="detail">
        ...
        <div class="container">
            <ul class="detail-header">
                ...
                /
                <li>
                    <NuxtLink to="/"><li class="detail-header-title">{{detail?.title}}</li></NuxtLink>
                </li>
            </ul>
            <div class="main-wrapper">
             <div class="images-wrapper">
                  <img class="detail-images" v-if="detail?.images?.length" :src="detail?.images[imagesIndex]" alt="">
              </div>
              <div class="detail-text-wrapper">
                <div class="price-wrapper">
                    <span>{{ Math.round(detail.price) }} $</span>
                    <span class="rating-count">{{ detail.rating }} <img class="rating" src="/assets/images/rating.svg" alt=""></span>
                </div>
                <h1 class="detail-title">{{ detail?.title }}</h1>
                <h2 class="detail-category">Category {{ detail?.category }}</h2>
                <div class="mini-images-menu">
                    <ul class="mini-images-wrapper">
                  <li v-for="(item, i ) in detail?.images" :key="item" :class="{'after-list': imagesIndex == i}"><img :src="item" alt="" @click="imagesIndex = i"></li>
                 </ul>
                </div>
                <p class="detail-desc">
                    {{ detail?.description }}
                </p>
                <span class="detail-brand">Brand {{ detail?.brand }} <img src="/assets/images/brand.svg" alt=""></span>
            ...
            <div class="rating-comment">
                <div class="rating-comment-wrapper">
                    <span class="reting-span" v-for="item in detail?.reviews" :key="item">{{ item?.comment }} <img class="rating" src="/assets/images/rating.svg" alt=""></span>
                </div>
            </div>
